# ✅ Full version with DEBUG enhancements
...
# ⬆️ Function to extract vehicle detection info from video (with DEBUGGING)
def extract_vehicle_data(video_path):
    ...
    for frame_num in range(min(frame_count, 200)):
        ret, frame = cap.read()
        if not ret:
            break
        ...
        img = torch.from_numpy(resized_frame).to(device).permute(2, 0, 1).float() / 255.0
        img = img.unsqueeze(0)

        # Get prediction from model
        pred_list = model(img, augment=False, visualize=False)
        pred = pred_list[0]
        ...
        if pred is not None and pred.shape[0] > 0:
            ...
            if pred.ndim == 2 and pred.shape[1] >= 6:
                ...
                # ✅ DEBUG: Show detected class IDs for each frame
                st.write(f"[DEBUG] Frame {frame_num}: Detected Class IDs - {class_ids}")
                ...
                st.warning(f"[DEBUG] Invalid prediction shape at frame {frame_num}")
                detections = sv.Detections.empty()
        else:
            st.warning(f"[DEBUG] No detections in frame {frame_num}")
            detections = sv.Detections.empty()
    ...
    # ✅ DEBUG: Final report of all class IDs seen
    st.info(f"[DEBUG] Unique classes detected in full video: {set(all_class_ids)}")
    return data, fps
