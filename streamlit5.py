...
# Use plain string paths instead of pathlib objects
video_options = {
    "sample1": "sample1.mp4",
    "sample2": "sample2.mp4",
    "sample3": "sample3.mp4"
...
VIDEO_PATH = str(video_options[selected_video_label])  # Ensure plain string

# Display the selected video
if os.path.exists(VIDEO_PATH):
    st.video(VIDEO_PATH)
else:
    st.warning(f"⚠️ File not found: {VIDEO_PATH}")

# Load your custom YOLOv5 model (ensure this is a string path)
...
model = torch.hub.load('ultralytics/yolov5', 'custom', path=str(CUSTOM_MODEL_PATH), force_reload=True)
...
def extract_vehicle_data(video_path):
    cap = cv2.VideoCapture(str(video_path))  # Ensure plain string
